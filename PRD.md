...
| 수업 기록 탭 | 강사 | ✅ 완료 | 전체 통계 개요(총 클래스/참여자/평균 참여율), 강의별 참여율 추이(차수별 참여율 바 + 트렌드 화살표(TrendingUp/Down/Minus) + %p 변화량 + 미니 트렌드 라인 SVG), 가장 어려웠던 질문(전 세션 통합, 정답률 기준 하위 5개, 3명 이상 응답 필터, AlertTriangle 아이콘, 강의명+차수+응답 수 표시), 최근 질문 목록. useRecentQuestions 훅이 Firebase에서 최근 10개 세션의 질문+참여자 데이터를 가져와 정답률 계산. 다크 모드 지원. StatsInsights.jsx 추출(TrendIndicator/MiniTrendLine/DifficultQuestions). 3뷰포트(1280/768/390) 확인, 콘솔 에러 0 |
