...
| # | Time | Task | Notes |
|---|------|------|-------|
| 48 | 2026-03-26 | 200줄+ 컴포넌트 추가 분리 — AssignmentsTab, QuestionLibraryView (6파일) | AssignmentsTab(297→89줄): AssignmentCard.jsx(65줄)+CreateAssignmentModal.jsx(156줄) 분리. QuestionLibraryView(303→192줄): LibraryQuestionCard.jsx(73줄)+LibrarySearchFilter.jsx(55줄) 분리. 4개 신규 파일 전부 200줄 미만. 빌드 통과 2400모듈. |
| 45 | 2026-03-26 | JoinPage 모바일 키보드 UX + StudentBottomBar 터치 UX 개선 (2파일) | JoinPage: useKeyboardDetect(visualViewport API) — 키보드 오픈 시 헤더/마스코트 spring collapse + 컴팩트 1줄 헤더 전환. 스티키 바텀 CTA — form[id]+button[form] 패턴으로 form 바깥에서 submit, safe-area-inset-bottom 적용. Avatar 인라인 미리보기 — 입력 필드 내부 left-3.5 float (isValid 시 paddingLeft animate). 에러/힌트 AnimatePresence mode=wait (tooShort/error/ready/hint 4가지 상태). StudentBottomBar: BTN_BASE/BTN_DEFAULT/BTN_ACTIVE 상수 통일 — 이전 복잡한 조건부 className 제거. 손들기 raised 상태: scale:0.8→1.8 pulse ring (opacity fade + repeat:Infinity), 흔들기 y:-2 bounce 추가. DM 도움 버튼 active DM 있을 때 text-emerald 상태색. 모든 버튼 whileTap scale 0.96→0.93 통일. |
...
| 47 | 2026-03-26 | 프레젠터 뷰 태블릿/폰 모바일 대응 (1파일) | SideNoticesPanel: md+ 기존 fixed w-72 패널 유지, md 미만 Hand/MessageSquare pill 토글 + AnimatePresence 슬라이드 드롭다운 (min(100vw-24px,288px)). ExitHint: pointer:coarse 감지 → 모바일 "탭하여 나가기" / 데스크탑 "ESC 또는 클릭" 자동. 메인 패딩 p-12→p-4/sm:p-8/lg:p-12. QROverlay bottom/right 3→md:5, 카드 w-56→md:w-64. PresentEmptyState text/gap 반응형. 집중모드 text-2xl→md:3xl→lg:4xl. 리더보드 px-2/md:px-0. TeamBattle 버튼 min-h-[48px]. |
| 46 | 2026-03-26 | 200줄+ 컴포넌트 추가 분리 (코드 품질) | QuestionList(259→112줄): QuestionItemContent+SortableItem → QuestionItem.jsx(151줄) 분리. StatsView(265→190줄): CoursePerformance+RecentQuestions → StatsCharts.jsx(87줄) 분리. 신규 파일 2개 모두 200줄 미만. 빌드 통과 |
