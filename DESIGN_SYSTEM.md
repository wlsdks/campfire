...
### 모바일 네비게이션 패턴
```
패턴 1 — 하단 탭 바 (Bottom Tabs)
  언제: 3-5개 동등한 최상위 섹션
  예시: 토스(홈/혜택/주식/전체), 당근(홈/동네/글쓰기/채팅/나)
  Pick: 강사 모바일 (진행/결과/참여/채팅)

패턴 2 — FAB (Floating Action Button)
  언제: 하나의 주요 생성 액션이 지배적
  예시: 당근(+ 글쓰기), Material Design 표준
  Pick: 검토 필요 — 질문 추가 버튼을 FAB로?

패턴 3 — 스크롤 피드 (Infinite Scroll)
  언제: 동일 유형 콘텐츠가 많을 때
  예시: 인스타그램, 트위터, 당근 피드
  Pick: 대시보드 세션 목록 (이미 스크롤)

패턴 4 — 상단 탭/세그먼트 (Top Tabs)
  언제: 같은 데이터를 다른 뷰로 보여줄 때
  예시: 카카오(친구/채팅/더보기), iOS 세그먼트 컨트롤
  Pick: 대시보드 (내 클래스/수업기록/보관함/더보기)

패턴 5 — 바텀 시트 (Bottom Sheet)
  언제: 컨텍스트 액션, 상세 옵션, 필터
  예시: Apple Maps, Google Maps, 토스 송금
  Pick: 검토 필요 — 세션 행 길게 누르기 → 바텀 시트(복제/삭제)

패턴 6 — 스와이프 액션 (Swipe Actions)
  언제: 리스트 아이템별 빠른 액션
  예시: iOS Mail(삭제/보관), 카카오(읽음/나가기)
  Pick: 검토 필요 — 세션 행 스와이프로 복제/삭제
```

### 래퍼런스 요약
| 출처 | 분류 | 핵심 교훈 |
|------|------|----------|
| 토스 | 미니멀+금융 | 배경색 대비, border 없음, 큰 숫자, 한 화면 한 액션 |
| Revolut | 미니멀+금융 | 카드 UI, 다크 테마, 숫자 중심 |
| 당근마켓 | 커머스+소셜 | FAB(+), 스와이프 액션, SEED 디자인 시스템 |
| 카카오톡 | 메시징 | 배경색 버블, 8px 구분선, 바텀 시트 |
| Telegram | 메시징 | 스와이프 답장, 빠른 액션, 부드러운 전환 |
| Linear | 생산성 | 모노크롬, spacing 위계, 키보드 우선 |
| Notion | 생산성 | 콘텐츠 = 인터페이스, 장식 0 |
| Things 3 | 생산성 | 제스처 기반, 아름다운 미니멀 |
| Apple HIG | 플랫폼 | 17pt 본문, 44pt 터치, inset grouped, backdrop-blur |
| Material 3 | 플랫폼 | FAB, 컨테이너 색상, 동적 컬러 |
| Kahoot | 교육 | 게이미피케이션, 카운트다운, 실시간 결과 |
| Mentimeter | 교육 | 실시간 워드클라우드, 투표 시각화 |
| shadcn/ui | 디자인 시스템 | Radix 기반 컴포넌트, 복사 가능한 코드 |
| Vercel Geist | 디자인 시스템 | 타이포+여백만으로 위계, 악센트 0 |
