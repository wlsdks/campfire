...
- **c) Race guard** ✅ `d12f482` — runTransaction first-wins + 2초 중복 dedup
...
### P2-2. resetAllQuestions 파괴적 동작에 확인 모달 ✅ `a75acbc` — 기존 모달에 in-flight 카운트 동적 표시 (손/긴급)
...
### P2-3. 스태프 모바일 currentMode 변화 미반영 ✅ `f1c90fa` — CurrentContext 상태바로 자동 갱신
...
### P2-4. answeredByRole 학생 측 표시 ✅ `6b6ed7b` — 표시는 기존, AnswerItem 공식 답변(강사/스태프) 시각 강조 추가
- **증상**: 답변자 신원이 staff 패널에서만 표시됨. 학생은 "강사가 답변" 같은 표식 못 봄
- **fix**: 학생 질문 카드에 답변자 역할 배지 (강사/스태프) 추가
- **공수**: S (45min)

### P2-5. BarChart indigo-600 → indigo-500 ✅ `a80ae45`
- **위치**: `src/features/visualization/components/BarChart.jsx:74,78`
- **fix**: indigo-500로 통일 (CTA-급 강도 회피)
- **공수**: XS (5min)

### P2-6. votes debounce 150ms 검토 ✅ `519f47e` — adaptive (≤10명 0ms, ≤50명 50ms, >50명 150ms)
...
- ✅ P2-7a Plinko `4c28178` (275줄 삭제, GameResultOverlay 라벨 정리)
- ✅ P2-7b Roulette `5b958d4` (303줄 삭제, MODE_MAP/lucide imports/design-tokens 정리)
- ✅ P2-7c+d PrizeDraw + SlotMachine 동일 iteration cut + orphan GameResultOverlay 제거
...
### P2-9. teams cut (D4) ✅ — 17개 파일에 걸쳐 wired 정리, AdminPage -6kb gzip
- TeamBattle/useMyTeam 등 일괄 제거
- **공수**: S (45min)

### P2-10. MysteryBox/HintReveal cut (D5) ✅ — games/* 4개 orphan 파일 523줄 제거
...
### P3-1. 다크모드 페어 누락 (~10건) 🚫 false positive — status dot은 functional 색상, /live는 강제 다크
...
### P3-2. 인라인 style hex 색상 정리 🚫 false positive — ReactionBar/JoinShow runtime dynamic color (Tailwind 불가)
...
### P3-3. useSession secondary fields 누락 정리 ✅ `467bd92` — activeAssignmentId 추가 (시상식 모드 fix)
...
### P3-4. 모바일 spot check ✅ `6f63c40` — Playwright 자동 스캔 (360/390/440px)
- JoinPage 3 viewport ✓ (마스코트/CTA/여백 일관)
- 강사 admin 모바일 (390px) ✓ (탭바/카드 정상)
- /live 모바일 ⚠️ 헤더 줄바꿈 발견 → fix `6f63c40` (반응형 padding/gap/hidden)
...
- ✅ **NEW-1** 강사 제출물 내용 미리보기 (사용자 요청, 백로그 외) `2a466ab` — SubmissionContentPreview 신설, expand에 통합. 이미지/코드/URL/설명/PRD/파일 모두 노출 + 풀모달.

### 다음 큐 (실행 순서)

1. ~~**P0-1c** 게임 race guard~~ ✅ `d12f482`
2. ~~**P2-5** BarChart indigo-600 → 500~~ ✅ `a80ae45`
3. ~~**P2-7a** Plinko cut~~ ✅ `4c28178`
4. ~~**P2-7b** Roulette cut~~ ✅ `5b958d4`
5. ~~**P2-7c** PrizeDraw cut~~ ✅ (P2-7b와 같은 commit batch)
6. ~~**P2-7d** SlotMachine cut~~ ✅ (P2-7b와 같은 commit batch)
7. ~~**P2-9** teams cut~~ ✅ (이전 commit, 17 파일 정리 + features/teams 폴더 삭제)
8. **P2-8** ⏸️ 공개 chat cut (1.5h) — **사용자 final confirm 필요** (skip)
9. ~~**P2-10** MysteryBox/HintReveal cut~~ ✅ 523줄 orphan 정리 ← 이번 iteration
10. ~~**P2-2** resetAllQuestions 확인 모달 강화~~ ✅ `a75acbc`
11. ~~**P2-3** 스태프 모바일 currentMode 동기화~~ ✅ `f1c90fa`
12. ~~**P2-4** answeredByRole 학생 표시~~ ✅ `6b6ed7b`
13. ~~**P2-6** votes debounce 적응형~~ ✅ `519f47e`
14. **P2-12** ⏸️ interactions/messaging 폴더 통합 (3~4h) — **사용자 확인 필요** (skip)
15. ~~**P3-1** 다크 페어~~ 🚫 false positive
16. ~~**P3-2** inline style~~ 🚫 false positive
17. ~~**P3-3** SECONDARY_FIELDS~~ ✅ `467bd92`
18. ~~**P3-4** 모바일 spot check + LiveHeader 반응형 fix~~ ✅ `6f63c40`
15. **P3-1~4** 정리 일괄 (다크모드/inline style/SECONDARY_FIELDS/모바일 회귀 spot)

### 멈춤 지점 (사용자 확인 필요)
- #8 P2-8: chat 전체 cut 시작 전 final confirm
- #14 P2-12: 통합 작업 시작 전
