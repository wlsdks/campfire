...
- **증상**: 강사가 데스크톱(/admin)으로만 진행 시 게임 결과(`gameResult`)가 Firebase에 안 써짐 → 학생/강사 모두 결과 오버레이 안 뜸
- **원인**: `src/app/routes/live/LivePage.jsx:73`의 `handleGameResult` → `useGameResult.publishResult` 만이 publish. 전자칠판이 없으면 트리거 안 됨
- **fix**: AdminPage 또는 게임 컴포넌트 자체에서 publishResult 호출. LivePage는 reflect만 하도록 책임 이전
...
- **위치**:
  - `src/features/report/components/ClassInsightCard.jsx:94` (indigo-600 원형 번호)
  - `src/components/ui/DrumrollOverlay.jsx:143-144` (amber 이중 원형 장식)
  - ~~`MysteryBoxPresenter.jsx:157`, `HintQuizPresenter.jsx:139`~~ — D5 (cut 예정)에 따라 P2-cut 작업 시 함께 처리
