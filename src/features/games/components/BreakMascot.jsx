 ...
 * Head and body share a bounce group — no gap between parts.
 * Head only gets independent rotation (tilt), never independent y movement.
 */
export default memo(function BreakMascot({ size = 140 }) {
  ...
  return (
    <svg width={size} height={size * 1.35} viewBox="0 0 140 190" fill="none" aria-hidden="true">
      {/* Walk: whole character sways left-right */}
      <motion.g
        animate={{ x: walk ? [-20, 0, 20, 0] : 0 }}
        transition={walk ? { duration: 3, repeat: Infinity, ease: 'easeInOut' } : { duration: 0.6 }}
      >
        {/* Bounce: head + body + arms move together (no gap) */}
        <motion.g
          animate={{
            y: dance ? [-5, 0, -5] : walk ? [-2, 0, -2] : stretch ? [-4, 0] : [0, -3, 0],
          }}
        ...
        >
          ...
          {/* === Body === */}
          <rect x="44" y="82" width="52" height="55" rx="20" fill="#FDE68A" stroke="#F59E0B" strokeWidth="2.5" />
          <circle cx="70" cy="108" r="10" fill="#FEF3C7" />

          {/* === Arms === */}
          <motion.rect
            x="30" y="88" width="12" height="28" rx="6" fill="#F59E0B"
            animate={{
          ...
          />
          <motion.rect
            x="98" y="88" width="12" height="28" rx="6" fill="#F59E0B"
            animate={{
          ...
          {/* === Head (only rotation, no independent y — shares bounce with body) === */}
          <motion.g
            animate={{ rotate: wave ? [0, 4, 0] : dance ? [-2, 2, -2] : 0 }}
            transition={wave || dance ? { duration: wave ? 0.4 : 0.5, repeat: Infinity, ease: 'easeInOut' } : { duration: 0.4 }}
            style={{ originX: '70px', originY: '70px' }}
          >
            {/* Neck bridge */}
            <rect x="52" y="68" width="36" height="20" rx="4" fill="#F59E0B" />
            ...
            {/* Eyes */}
            <motion.ellipse cx="61" cy="46" rx="3.5" ry="4" fill="#1E293B"
              animate={{ scaleY: [1, 1, 0.1, 1, 1] }}
              transition={{ duration: 4, repeat: Infinity, times: [0, 0.45, 0.5, 0.55, 1] }}
            />
            <circle cx="59.5" cy="44.5" r="1.5" fill="white" />
            <motion.ellipse cx="79" cy="46" rx="3.5" ry="4" fill="#1E293B"
              animate={{ scaleY: [1, 1, 0.1, 1, 1] }}
              transition={{ duration: 4, repeat: Infinity, times: [0, 0.45, 0.5, 0.55, 1] }}
            />
