    ...
    <div ref={containerRef} className="mt-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden shadow-sm">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {isActive && (
            <span className="text-xs font-medium text-white bg-indigo-600 px-1.5 py-0.5 rounded">
              {teamCount}팀 진행중
            </span>
          )}
        </div>
                    ...
                    <div className="flex gap-2">
                      {[2, 3, 4].map((n) => (
                        <button
                          ...
                          className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all active:scale-[0.96] ${
                            selectedCount === n
