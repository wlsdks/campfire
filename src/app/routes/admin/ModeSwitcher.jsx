...
import { Target, Ticket, Trophy, Swords, Gift, Dices, Coffee, X, ChevronDown, CircleDot, MessageSquare, Zap, Award, Eye, UserCircle, Activity, BarChart3, Timer, HelpCircle, Lightbulb } from 'lucide-react';
...
export default memo(function ModeSwitcher({ currentMode, isSpecialMode, totalTickets, leaderboard, modeOpen, onToggle, onSwitchMode, teamBattleActive = false }) {
  ...
  const modeGroups = [
    ...
    { label: '퀴즈 연출', items: [
      { mode: 'mysteryBox', label: '미스터리 박스', icon: HelpCircle },
      { mode: 'hintReveal', label: '힌트 퀴즈', icon: Lightbulb },
    ]},
    { label: '기타', items: [
