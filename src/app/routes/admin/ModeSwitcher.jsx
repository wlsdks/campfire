...
import { Target, Ticket, Trophy, Swords, Gift, Sparkles, X, ChevronDown } from 'lucide-react';
        ...
        <div className="flex items-center gap-2">
          ...
          {isSpecialMode && (
            <span className="text-xs font-medium text-slate-600 dark:text-slate-200 bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">
              {currentMode === 'roulette' ? '돌림판' : currentMode === 'lottery' ? '제비뽑기' : currentMode === 'prizeDraw' ? '경품 추첨' : currentMode === 'slotMachine' ? '777 슬롯' : currentMode === 'teamBattle' ? '팀 스코어보드' : '리더보드'}
            </span>
          )}
        </div>
