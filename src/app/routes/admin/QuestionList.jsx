      ...
      <div className="flex items-start gap-2">
        ...
        {!readOnly && (
          <div className="flex gap-1 shrink-0" onPointerDown={stopDrag}>
            {!isActive ? (
              <button onClick={() => onActivate(qId)} className="p-1.5 rounded-md bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-500 text-white transition-all active:scale-90" aria-label="질문 활성화">
                <Play size={12} />
              </button>
            ) : (
              <>
                {isQuiz && !q.revealedAt && (
                  <button onClick={() => onReveal(qId)} className="p-1.5 rounded-md bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-500 text-white transition-all active:scale-90" aria-label="정답 공개">
                    <Check size={12} />
                  </button>
                )}
                {isQuiz && q.revealedAt && (
                  <button onClick={onShowLeaderboard} className="p-1.5 rounded-md bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-500 text-white transition-all active:scale-90" aria-label="리더보드 보기">
                    <Trophy size={12} />
                  </button>
                )}
