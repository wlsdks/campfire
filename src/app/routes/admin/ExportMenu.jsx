...
export default function ExportMenu({ session, participants, scores }) {
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        onClick={() => setOpen((p) => !p)}
        className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-slate-900 hover:bg-slate-800 text-white text-sm font-medium transition-colors active:scale-[0.97]"
        aria-expanded={open}
        aria-label="CSV 내보내기"
      >
        <Download size={16} />
