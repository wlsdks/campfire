          ...
          <div className="relative">
            ...
            {searchQuery && (
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 dark:text-slate-500 dark:hover:text-slate-300 transition-colors"
                aria-label="검색어 지우기"
              >
                <X size={14} />
              </button>
      ...
      {questions.length === 0 && !showForm ? (
        ...
        <div className="space-y-2">
          ...
          {filtered.length === 0 && questions.length > 0 && (
            ...
            >
              <Search size={20} className="text-slate-300 dark:text-slate-600 mx-auto" />
              <p className="text-slate-400 text-sm">일치하는 질문이 없습니다</p>
              <p className="text-slate-300 dark:text-slate-500 text-xs">검색어나 필터를 변경해보세요</p>
            </motion.div>
          )}
        </div>
      )}
